...
export class CdiscountMerchantConnector {
  private merchantId: string = 'cdiscount-pilot';
  private merchantName: string = 'Cdiscount (Flux Pilote)';
  private feedUrl: string = '/cdiscount-feed.json';
  ...
  public async fetchAndSync(): Promise<{ success: boolean; itemsProcessed: number; offersUpdated: number; message: string }> {
    ...
    try {
      ...
      const localFilePath = path.join(process.cwd(), 'public', 'cdiscount-feed.json');
      if (fs.existsSync(localFilePath)) {
        const fileContent = fs.readFileSync(localFilePath, 'utf-8');
        rawData = JSON.parse(fileContent);
      } else {
        throw new Error(`Flux Cdiscount introuvable : ${localFilePath}`);
      }

      if (!rawData || !Array.isArray(rawData.items)) {
        throw new Error('Format de flux Cdiscount invalide : items[] manquant');
      }
      ...
      for (const item of rawData.items) {
        ...
        const offer: LiveOffer = {
          ...
          seller: item.merchantSeller || 'Cdiscount Officiel',
          price: Number(item.price),
          originalPrice: Number(item.originalPrice || item.price),
          currency: item.currency || 'EUR',
          condition: condition,
          conditionLabel: condition === 'refurbished'
            ? 'Reconditionné Cdiscount Seconde Vie (Garantie 2 ans)'
          ...
          deliveryInfo: item.deliveryType || 'Livraison standard gratuite',
      ...
      const log: ConnectorExecutionLog = {
        ...
        status: 'success',
        message: `Cdiscount : ${offersUpdated} offres synchronisées avec succès (${nowFormatted})`,
      };
      db.addLog(log);
      ...
      const errorMsg = err?.message || 'Erreur inconnue connecteur Cdiscount';
      const log: ConnectorExecutionLog = {
        timestamp: nowISO,
        merchantId: this.merchantId,
        itemsProcessed: 0,
        ...
        status: 'error',
        message: `Échec connecteur Cdiscount : ${errorMsg}`,
      };
      db.addLog(log);
...
export const cdiscountConnector = new CdiscountMerchantConnector();
